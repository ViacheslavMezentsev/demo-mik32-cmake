...
I2C_HandleTypeDef hi2c0 = { 0 };
DMA_InitTypeDef hdma    = { 0 };
DMA_ChannelHandleTypeDef hdma_ch0_tx = { 0 };
DMA_ChannelHandleTypeDef hdma_ch1_rx = { 0 };
...
int main()
{
    ...
    while ( 1 )
    {
        ...
        // Отправка данных по адресу I2C_SLAVE_ADDRESS.
        xprintf( "\nMaster_Transmit\n" );

        // Сброс I2C перед отправкой данных. 
        HAL_I2C_Reset( &hi2c0 );

        // Передача данных по I2C.
        HAL_StatusTypeDef error_code = HAL_I2C_Master_Transmit_DMA( &hi2c0, I2C_SLAVE_ADDRESS, data, sizeof( data ) );
        ...
        error_code = HAL_I2C_Master_Receive_DMA( &hi2c0, I2C_SLAVE_ADDRESS, data, sizeof( data ) );
...
/**
 * @brief   Инициализация канала DMA 0.
 * 
...
/**
 * @brief   Инициализация канала DMA 1.
 * 
 * @param   hdma Указатель на структуру инициализации DMA.
 */
static void DMA_CH1_Init( DMA_InitTypeDef* hdma )
{
    ...
    hdma_ch1_rx.ChannelInit.ReadSize = DMA_CHANNEL_SIZE_BYTE; 
    hdma_ch1_rx.ChannelInit.ReadBurstSize = 0;
    ...
    hdma_ch1_rx.ChannelInit.WriteSize = DMA_CHANNEL_SIZE_BYTE;
    hdma_ch1_rx.ChannelInit.WriteBurstSize = 0;
