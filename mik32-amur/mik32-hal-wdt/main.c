 ...
 * В начале программа инициализирует такие периферии как UART и WDT. 
 * Затем по UART отправляется строка "Start", что говорит о успешном старте программы. 
 * После этого запускается таймер WDT, который отсчитывает 1000 мс. 
 * Затем значение счетчика таймера сбрасывает функцией HAL_WDT_Refresh. После этого
 ...
 *
 * Настраивает сторожевой таймер с использованием внешнего 32 кГц осциллятора и
 * устанавливает время перезагрузки в 1000 мс.
 */
void WDT_Init()
{
    hwdt.Instance = WDT;
