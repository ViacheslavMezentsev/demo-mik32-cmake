...
int main()
{
    ...
    while ( 1 )
    {
        ...
        {
            // Включение SPI и выбор ведомого устройства CS0.
        ...
        {
            xprintf( "SPI_Error %d, OVR %d, MODF %d\n", SPI_Status, hspi0.ErrorCode & HAL_SPI_ERROR_OVR,
                hspi0.ErrorCode & HAL_SPI_ERROR_MODF );
            HAL_SPI_ClearError( &hspi0 );
        }

        // Конец передачи в ручном режиме управления CS.
        // Если используется ручное управление CS, то отключаем ведомое устройство CS0 и выключаем SPI.
        if ( hspi0.Init.ManualCS == SPI_MANUALCS_ON )
        {
 ...
 * 
 * Настраивает SPI0 в режиме ведущего с частотой 500KHz, используя делитель частоты 64.
 * Используется ручное управление CS, выбирается ведомое устройство CS0.
 */
static void SPI0_Init( void )
{
