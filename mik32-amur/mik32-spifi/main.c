...
/**
 * @brief   Инициализация SPIFI.
 * 
 * Эта функция инициализирует SPIFI, настраивает необходимые GPIO и регистры для работы с внешней флеш памятью.
 */
void spifi_init()
{
...
/**
 * @brief   Ожидание завершения операции SPIFI.
 * 
 * Эта функция ожидает, пока бит INTRQ в регистре STAT не станет равным 1, что означает завершение операции SPIFI.
 * 
 * @param spifi    Указатель на структуру конфигурации SPIFI.
 * @param timeout  Максимальное время ожидания в циклах.
 * 
 * @return  Возвращает 1, если операция завершена успешно, иначе 0.
 */
int SPIFI_WaitIntrqTimeout( SPIFI_CONFIG_TypeDef* spifi, uint32_t timeout )
{
    while ( timeout-- && !( spifi->STAT & SPIFI_CONFIG_STAT_INTRQ_M ) );

    return ( spifi->STAT & SPIFI_CONFIG_STAT_INTRQ_M ) ? 1 : 0;
}


/**
 * @brief   Разрешение записи в SPIFI.
 * 
 * Эта функция отправляет команду разрешения записи в SPIFI, что позволяет выполнять операции записи в память.
 * 
 * @note    Перед выполнением операций записи необходимо вызвать эту функцию.
 */
void write_enable()
{
    /*
...
/**
 * @brief   Чтение регистра состояния SPIFI.
 * 
 * Эта функция читает состояние регистра SREG1, который содержит информацию о текущем состоянии SPIFI.
 * 
 * @return  Возвращает значение регистра SREG1.
 */
uint8_t read_sreg_1()
{
    uint8_t read_sreg = 0;
 ...
 * @brief   Ожидание завершения операции SPIFI.
 * 
 * Эта функция ожидает, пока бит SREG1_BUSY не станет равным 0, что означает, что SPIFI не занят выполнением операции.
 */
void wait_busy()
{
    xprintf( "Wait\n" );
    ...
    {
        sreg = read_sreg_1();
        if ( !( sreg & SREG1_BUSY ) ) break;
    }
}


/**
 * @brief   Стирание всей памяти SPIFI.
 * 
 * Эта функция выполняет полное стирание всей памяти SPIFI, что приводит к удалению всех данных.
 */
void chip_erase()
{
    xprintf( "Start erase\n" );
...
/**
 * @brief   Чтение данных из SPIFI.
 * 
 * @param address     Адрес, с которого читаются данные.
...
/**
 * @brief   Запись страницы в SPIFI.
 * 
 * @param ByteAddress  Адрес, куда записываются данные.
...
void erase()
{
    ...
    chip_erase();   // Выполняем стирание всей памяти
    wait_busy();    // Ожидаем завершения операции стирания
}


/**
 * @brief   Запись данных в SPIFI.
 * 
 * @param address   Адрес, куда записываются данные.
 * @param data      Данные для записи.
