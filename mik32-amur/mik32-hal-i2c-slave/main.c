...
int main()
{
    ...
    while ( 1 )
    {
        /* Чтение данных */
        xprintf( "\nSlave_read\n" );
        HAL_StatusTypeDef error_code = HAL_I2C_Slave_Receive( &hi2c0, data, sizeof( data ), 5 * I2C_TIMEOUT_DEFAULT );
...
/**
 * @brief   Обработчик события I2C в режиме ведомого.
 * 
 * @param hi2c Указатель на структуру I2C_HandleTypeDef.
 * @param pData Указатель на данные, которые ведомый должен обработать.
 * @param DataSize Размер данных, которые ведомый должен обработать.
 * @param ByteCount Количество байт, которые ведомый должен обработать.
 */
