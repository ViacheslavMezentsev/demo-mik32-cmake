 ...
 * Нулевой канал таймера используется в режиме захвата. 
 * На Port0_0 подается периодичный сигнал (ШИМ), частота которого измеряется и выводится в UART0.
 */
...
/**
 * @brief Инициализация таймера 32-битного.
 *
 * Настраивает таймер 32-битный с использованием прерываний.
 */
static void Timer32_Init( void )
{
 ...
 *
 * Обрабатывает прерывания от таймера 32-битного, включая переполнение, сравнение и захват.
 */
void trap_handler()
{  
